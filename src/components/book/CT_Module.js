...
export function createCTStructure(ficheP){
    var i,j,k;
    var tempTab=[];
    var cur_module = '';
    var cur_chapitre = '';
    ...
    while(i<cpt_module){
        ...
        while(j<tempTab.length){
            ...
            while(j<tempTab.length && cur_chapitre ==  tempTab[j].chapitre){
                if(lessons.length==0) lessons = 'lesson'+(k+1)+'_'+ficheP[j].lecon+'_'+ficheP[j].status;
                else lessons = lessons +'*'+ 'lesson'+(k+1)+'_'+ficheP[j].lecon+'_'+ficheP[j].status;                
                j++; k++;
            }
